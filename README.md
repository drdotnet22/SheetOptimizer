...
The winning layout is picked by comparing, in order: fewest unplaced parts, fewest full sheets, least total sheet area, **least small-scrap area** (leftovers under the minimum offcut size), then - and only between layouts whose scrap is within one square foot of each other (`NestingOptions.CutTieScrapTolerance`) - the **fewest saw cuts**, and finally the **biggest single leftover piece**. In plain terms: waste always wins, but between near-equal layouts, the one that's less work at the saw is chosen. Estimated cut counts are shown per sheet and per material on the results page, alongside the winning strategy.
